  ...
  login = async props => {
    if (
      ...
      await fetch(
        ngrok +
          '/api/custom-user/' +
          this.state.credentials.username.toString(),
        {
          method: 'GET',
          headers: {
        ...
        .then(data => {
          if (!data.length()) {
            Alert.alert('A user with this credentials does not exist! ', [
              {text: 'OK'},
            ]);
          } else {
            ...
            AsyncStorage.setItem(
              'first_name',
              JSON.stringify(this.current_user.first_name),
            );
            AsyncStorage.setItem(
              'last_name',
              JSON.stringify(this.current_user.last_name),
            );
            AsyncStorage.setItem(
              'username',
              JSON.stringify(this.current_user.username),
            );
            AsyncStorage.setItem(
              'email',
              JSON.stringify(this.current_user.email),
            );
            AsyncStorage.setItem(
              'about',
              JSON.stringify(this.current_user.about),
            );
            AsyncStorage.setItem(
              'image',
              JSON.stringify(this.current_user.image),
            );
            AsyncStorage.setItem(
              'level',
              JSON.stringify(this.current_user.level),
            );
